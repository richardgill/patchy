...
describe("updateJsoncField", () => {
  it("should add a new field to JSON", () => {
    const input = `{
  "upstream_url": "https://github.com/foo/bar"
}`;
    const result = updateJsoncField(input, "upstream_dir", "my-repo");

    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.content).toContain('"upstream_dir": "my-repo"');
      expect(result.content).toContain('"upstream_url"');
    }
  });
  ...
  it("should update an existing field", () => {
    const input = `{
  "upstream_url": "https://github.com/foo/bar",
  "upstream_dir": "old-repo"
}`;
    const result = updateJsoncField(input, "upstream_dir", "new-repo");

    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.content).toContain('"upstream_dir": "new-repo"');
